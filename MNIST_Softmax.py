...
class SoftmaxModel(nn.Module):
    ...
    def reshape(self, flat_gradient):
        layers = []
        layers.append(
            torch.from_numpy(np.reshape(flat_gradient[0:self.D_in * self.D_out], 
                                        (self.D_out, self.D_in))).type(
                torch.FloatTensor))
        layers.append(
            torch.from_numpy(flat_gradient[self.D_in * self.D_out:self.D_in * self.D_out + self.D_out])
...
def run(f):
    ...
    node=p2p.Node()
    filename1 = "./log/krum/loss/" + "loss_" + str(node.PORT) + ".txt"
    filename2 = "./log/krum/error/" + "Test_error_" + str(node.PORT) + ".txt"
    filename3 = "./log/krum/time/" + "time_correspond" + str(node.PORT) + ".txt"
    filename4 = "./log/krum/time/" + "time_compute" + str(node.PORT) + ".txt"
