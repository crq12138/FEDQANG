...
def gaussian_noise(grad):
    ...
    for i in range(noise_grad.size):
        laplace1 = np.random.laplace(0,2*0.01/epsilon*abs(np.mean(noise_grad)))
        noise_grad[i]+=laplace1
    return noise_grad
def BFT(grad):
    BFT_grad = grad
...
def run():
    ...
    node=p2p.Node()
    filename1 = "./log/10/loss/" + "loss_" + str(node.PORT) + "_krum.txt"
    filename2 = "./log/10/error/" + "Test_error_" + str(node.PORT) + "_krum.txt"
    ...
    for iter in range(iter_time):
        ...
        krum_grad1 = krum(grad_recv, node_size - Num_Bft(node_size) - 2, node_size - Num_Bft(node_size))
