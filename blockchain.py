...
class Blockchain:
    ...
    def initialize_hash_ring(self):
        ...
        for node_address in sorted_nodes:
            ...
            # weight = p2p.quality_score_dict[node_address.split(":", 1)[1].strip()]
            weight = 70
            # print(weight)
            # time.sleep(3000)
    ...
    def create_block(self,tensor, quality_score_dict = b'', transfer_dict = b'') -> grpc_pb2.Block:
    ...
    def consensus_process(self, krum_grad_bytes, quality_score_dict, transfer_dict):
        # global final_block
        ...
        # 1. 创建备选区块
        candidate_block = self.create_block(krum_grad_bytes, pickle.dumps(quality_score_dict), pickle.dumps(transfer_dict))
...
class Consensus(grpc_pb2_grpc.ConsensusServicer):
    ...
    def BroadFinalBlock(self, request, context):
        ...
        p2p.transfer_dict = pickle.loads(request.block.transfer)
