...
def plot_data_contribution_ratio_avg():
    ...
    ax.plot(grid_x, iid_ratio_resampled, label=f'独立同分布参与方', 
            color='tab:green', linewidth=2.0, marker='o', markersize=5)
    
    # 绘制 Non-IID (低质量)
    ax.plot(grid_x, non_iid_ratio_resampled, label=f'非独立同分布参与方', 
    ...
    ax.set_ylabel('数据贡献比例', fontsize=16)
    ax.set_xlim(0, 100) 
    ax.set_ylim(-0.05, 1.05) 
    ax.legend(fontsize=16, loc='upper right', bbox_to_anchor=(0.995, 0.93))
