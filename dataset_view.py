...
def draw_stacked_bar_chart(data_path, num_clients, num_classes):
    ...
    for client_id in range(num_clients):
        client_file = os.path.join(data_path, f"cifar10_{client_id}.npy")
        # client_file = os.path.join(data_path, f"cifar10_unif_5000_{client_id}.npy")
    ...
    print(client_label_distributions)
    # 转换为 NumPy 数组便于处理
    ...
    plt.savefig(os.path.join(save_path, 'cifar10_label_distribution_clients_unif.png'))
    plt.show()
    ...
    plt.savefig(os.path.join(save_path, 'cifar10_client_distribution_labels_unif.png'))
    plt.show()
...
# 参数设置
# data_directory = "./cifar-10-batches-py/cifar10/"  # 替换为你的数据文件夹路径
data_directory = "./cifar-10-batches-py/cifar10/"  # 替换为你的数据文件夹路径
num_clients = 7  # 客户端数量
