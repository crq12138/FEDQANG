...
port = str(p2p.PORT)
# filename = "./log/cnn/non-iid/tm/datasize/" + "traindata_" + port + ".txt"
filename = "./log/cnn/mixed/1_5n_5i/datasize/" + "traindata_" + port + ".txt"
...
def decentralized_game(client, Loss, iter):
    ...
    if(iter == 0):
        datasize = int(client.trainset.n/2)
    else:
        datasize = data_loop
    convergence_self = 0
    # iteration = 0
    ...
    while True:
        ...
        new_datasize = solve_optimal_data_contribution(data_contribution, quality_score_dict, port, None, Loss, 60.0, max_data_size)
...
def solve_optimal_data_contribution(
    ...
    # 若未提供 p_n_dict, 则为所有参与方设定一个默认 p_n=7.9
    if p_n_dict is None:
        p_n_dict = {pid: 79 for pid in all_data_contributions.keys()}
    ...
    def compute_loss_function(x_n, participant):
        ...
        _, _, loss_dec = compute_loss_decrease(participant, x_n, 
                                               ...
                                               k)
        # 计算效用
        p_n_val = p_n_dict[participant] if participant in p_n_dict else 79
