...
class MNISTCNNModel(nn.Module):
    def __init__(self, D_in=None, D_out=None):
        super(MNISTCNNModel, self).__init__()
        # 两个卷积层，kernel_size=5，padding=2，这样 28×28 经过池化后变为 14×14、7×7
    ...
    def forward(self, x):
        ...
        # 第 1 个卷积 + ReLU + 2×2 max pooling
        x = F.relu(self.conv1(x))        # 输出: [batch_size, 32, 28, 28]
        x = F.max_pool2d(x, 2)           # 输出: [batch_size, 32, 14, 14]
        
        # 第 2 个卷积 + ReLU + 2×2 max pooling
        x = F.relu(self.conv2(x))        # 输出: [batch_size, 64, 14, 14]
        x = F.max_pool2d(x, 2)           # 输出: [batch_size, 64, 7, 7]
        ...
        # 输出层，这里可以直接返回 logits，然后在训练中用 CrossEntropyLoss
        # 如果想要显示使用 Softmax，也可使用 F.softmax(x, dim=1) 或 F.log_softmax
        x = self.fc2(x)
        return x
