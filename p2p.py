...
class Node:
    def __init__(self):
        ...
        self.blockchain = blockchain_instance  # 需要在主程序中创建一个全局的Blockchain实例
    ...
    def add(self, target):
        ...
        if type(target) == str:
            ...
            self.add("%s:%s" % target)
        # self.blockchain.initialize_hash_ring()  # 更新哈希环
    ...
    def send(self, task, message, target_node, datasize = 0, cost_fuction = 0):
        """发送消息到指定的目标节点"""
        try:
            ...
            if task_type == bc_enum.DESCOVERY:
                ...
                if task_sub == bc_enum.EXCHANGENODE:
                    ...
                    response = stub.ExchangeGrad(grpc_pb2.Parameter(para=message, datasize = datasize, port = PORT, cost = cost_fuction))
    ...
    def send_grad(self, grad, target_node, datasize, cost):
        """发送梯度到指定的目标节点"""
        # grad_bytes = pickle.dumps(grad)
        task = bc_enum.SERVICE * bc_enum.DESCOVERY + bc_enum.EXCHANGEGRAD
        # print(type(datasize))
        self.send(task, grad, target_node, datasize, cost)
