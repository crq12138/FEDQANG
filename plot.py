...
def plot_multiple_clients(data_dict, title, xlabel, ylabel, save_path=None):
    ...
    for client_id, (iterations, values) in data_dict.items():
        if idx < 3:
            plt.plot(iterations, values, linestyle='-', label=f'Client {idx} (non-iid)')
        else:
            plt.plot(iterations, values, linestyle='-', label=f'Client {idx} (iid)')
        idx += 1
    plt.title(title)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
...
quality_data = {}
datasize_data = {}
read_dir = './log/cnn/mixed/test'
save_dir = './result/cnn/MNIST/mixed/test'

datasize = [5949, 6265, 5851, 5000, 5000, 5000, 5000, 5000, 5000, 5000]
datasize_dict = {client_id: size for client_id, size in zip(client_ids, datasize)}
# print(datasize_dict)
...
for client_id in client_ids:
    ...
    loss_log_path = read_dir + f'/loss/loss_{client_id}.txt'
    error_log_path = read_dir + f'/error/Test_error_{client_id}.txt'
    quality_log_path = read_dir + f'/quality_score/Quality_score_{client_id}.txt'
    datasize_log_path = read_dir + f'/datasize/traindata_{client_id}.txt'
    ...
    datasize_iterations, datasize_values = read_log_file(datasize_log_path)

    # 获取客户端的总数据集大小
    total_datasize = datasize_dict.get(client_id, 1)  # 预防异常情况，默认总大小为1
    
    # 计算贡献比例
    datasize_values = [val / total_datasize for val in datasize_values]
    datasize_data[client_id] = (datasize_iterations, datasize_values)


    # 只提取前200轮数据
    loss_iterations, loss_values = loss_iterations[:200], loss_values[:200]
    error_iterations, error_values = error_iterations[:200], error_values[:200]
    quality_iterations, quality_values = quality_iterations[:-10], quality_values[:-10]
    datasize_iterations, datasize_values = datasize_iterations[:200], datasize_values[:200]
...
plot_multiple_clients(
    ...
    save_path=save_dir + '/accuracy_plot.png'
)
...
#     save_path=save_dir + '/error_plot.png'
# )
# # 绘制多个客户端的数据大小曲线
# plot_multiple_clients(
#     datasize_data,
#     'Data Size over Iterations',
#     'Iteration',
#     'Data Size',
#     save_path=save_dir + '/datasize_plot.png'
# )
...
plot_multiple_clients(
    ...
    save_path=save_dir + '/quality_plot.png'
)
